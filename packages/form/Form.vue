<template>
  <el-form v-bind="$attrs">
    ...
    <el-form-item
      ...
      <slot v-else-if="$scopedSlots[item.prop] || $slots[item.prop]" :name="item.prop" v-bind="item.props" v-on="item.listeners"></slot>
      <component v-else :is="item.component || 'el-input'" v-bind="item.props" v-on="item.listeners" v-model="model[item.prop]"/>
    </el-form-item>
    <slot></slot>
  </el-form>
</template>
