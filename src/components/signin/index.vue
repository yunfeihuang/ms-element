<template>
  <div class="e-signin">
    <el-form ref="form" :model="form" @submit.native.prevent="handleSubmit">
      <legend v-if="!$slots.title">{{title}}</legend>
      <slot name="title" v-else></slot>
      <slot name="prepend" v-bind="{form: form}"></slot>
      <el-form-item prop="account" :rules="[ {required: true, message: placeholder} ]">
        <el-input v-model.trim="form.account" v-bind="accountProps" :placeholder="placeholder" @change="handleChange">
          <e-icon v-if="accountType.length==1&&accountType.indexOf('email')>-1" slot="prefix" class="el-input__icon">&#xe619;</e-icon>
          <e-icon v-else-if="accountType.length==1&&accountType.indexOf('mobile')>-1" slot="prefix" class="el-input__icon">&#xe67e;</e-icon>
          <e-icon v-else slot="prefix" class="el-input__icon">&#xe604;</e-icon>
        </el-input>
      </el-form-item>
      <el-form-item prop="password" v-if="passwordProps" :rules="[
        {required: true, message: passwordProps.placeholder}
        ]">
        <e-password v-model.trim="form.password" v-bind="passwordProps" @change="handleChange"/>
      </el-form-item>
      <el-form-item prop="smsCode" v-if="smsCodeProps" :rules="[
        {required: true, message: smsCodeProps.placeholder}
        ]">
        <e-sms-code :to="form.account" v-model.trim="form.smsCode" v-bind="smsCodeProps"/>
      </el-form-item>
      <el-form-item prop="code" v-if="codeProps" :rules="[
        {required: true, message: codeProps.placeholder}
        ]">
        <e-img-code ref="imgCode" v-model.trim="form.code" v-bind="codeProps"/>
      </el-form-item>
      <div v-if="errorMessage" class="el-form-item__error e-signin-error">
        <!-- {{errorMessage}} -->
        <el-alert :title="errorMessage" type="error" show-icon></el-alert>
      </div>
      <el-form-item class="is-argument" v-if="$slots.argument" prop="argument">
        <el-checkbox v-model="form.argument">
          <small>{{accpetText}}</small>
        </el-checkbox>
        <slot name="argument"></slot>
      </el-form-item>
      <slot name="append" v-bind="{form: form}"></slot>
      <el-button type="primary" :disabled="!form.argument" :loading="loading" native-type="submit">{{submitText}}</el-button>
    </el-form>
    <div class="e-signin-links" v-if="$slots.links">
      <slot name="links"></slot>
    </div>
  </div>
</template>
<script>
import form from '@/mixins/form'
export default {
  mixins: [form],
  props: {
    title: {
      type: String,
      default: '用户登录'
    },
    accountType: {
      type: Array,
      default () {
        return ['mobile', 'email']
      }
    },
    accountProps: {
      type: Object
    },
    passwordProps: {
      type: Object,
      default () {
        return {
          placeholder: '请输入密码'
        }
      }
    },
    codeProps: {
      type: Object,
      default () {
        return {
          placeholder: '请输入验证码'
        }
      }
    },
    smsCodeProps: {
      type: Object
    },
    submitText: {
      type: String,
      default: '立即登录'
    },
    action: {
      type: String
    },
    customForm: {
      type: Object
    },
    onSubmit: {
      type: Function
    },
    accpetText: {
      type: String,
      default: '我已阅读并接受'
    }
  },
  data () {
    return {
      loading: false,
      errorMessage: '',
      form: {
        ...this.customForm,
        account: '',
        password: '',
        code: '',
        smsCode: '',
        argument: true
      }
    }
  },
  computed: {
    placeholder () {
      if (this.accountProps && this.accountProps.placeholder) {
        return this.accountProps.placeholder
      }
      let tmp = {
        mobile: '手机号',
        email: '邮箱'
      }
      let names = this.accountType.map(item => {
        return tmp[item]
      })
      return `请输入${names.join('/')}`
    }
  },
  methods: {
    handleChange () {
      this.errorMessage = ''
    },
    submit () {
      if (this.action) {
        this.loading = true
        this.$axios({
          url: this.action,
          method: 'POST',
          data: JSON.parse(JSON.stringify(this.form))
        }).then(res => {
          this.loading = false
          this.$emit('success', res)
        }).catch(res => {
          this.loading = false
          this.$emit('error', res)
          this.errorMessage = res.msg
          this.$refs.imgCode && this.$refs.imgCode.refresh()
        })
      } else if (this.onSubmit) {
        this.loading = true
        this.onSubmit(JSON.parse(JSON.stringify(this.form)), this).then(res => {
          this.loading = false
          this.$emit('success', res)
        }).catch(res => {
          this.loading = false
          this.$emit('error', res)
          this.errorMessage = res.msg
          this.$refs.imgCode && this.$refs.imgCode.refresh()
        })
      } else {
        this.$emit('success')
      }
    }
  }
}
</script>
